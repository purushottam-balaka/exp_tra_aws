...
window.addEventListener('DOMContentLoaded',async()=>{
    ...
    await axios.get(`http://3.110.32.173:9000/get_expenses?page=${page}&&rows=${row}`,{headers:{'Authorization':token}})
    .then(({data:{products,prime, ...pageData}})=>{
        ...
        if(primeUser===true){
                    ...
                    try{
                        ...
                        const res=await axios.get('http://3.110.32.173:9000/leaderboard')
            ...
            const res=await axios.get('http://3.110.32.173:9000/purchase_premium',{headers: {'Authorization':token}})
            var options={
                "key":res.data.key_id,
                "order_id":res.data.order.id,
                "handler":async function(res){
                const d1= await axios.post('http://3.110.32.173:9000/update_purchase',{
            ...
            rzp1.on('payment.failed',async function(){
                try{
                    ...
                    const response=await axios.post('http://3.110.32.173:9000/update_purchase',{
                        'order_id':key,
                        'payment_id': null,
                    },{ headers:{"Authorization":token}})
...
function showExpeneseOnScreen(exp){
    ...
    editButton.onclick=async()=>{
        try{
            const token=localStorage.getItem('token');
            const user=await axios.get(`http://3.110.32.173:9000/edit_expense/${editButton.id}`,{headers:{'Authorization':token}})
            ...
            await axios.delete(`http://3.110.32.173:9000/delete_expense/${editButton.id}`,{headers:{'Authorization':token}})    
    ...
    delButton.onclick=async()=>{
        ...
        const token=localStorage.getItem('token')
        parentEle.removeChild(chiledEle)
        await axios.delete(`http://3.110.32.173:9000/delete_expense/${delButton.id}`,{headers:{'Authorization':token}})
         .then((resp)=>{
...
async function getProdcuts(page){
    ...
    try{
        ...
        const token=localStorage.getItem('token');
         await axios.post('http://3.110.32.173:9000/add_expense',expenseDetails,{headers: {'Authorization':token}})
         .then((res)=>{
            window.location.reload();
         }).catch((err)=>{
            console.log(err)
    ...
    async function downloadFile(e){
       axios.get('http://3.110.32.173:9000/download',{headers:{Authorization:token}})
       .then((resp)=>{
        if(resp.status===200){
